    ...
    public class HealthManagerLevel2 : MonoBehaviour
    {
        ...
        void Start()
        {
            if (PlayerPrefs.HasKey("PlayerHealth"))
            {
                ...
                Debug.Log("Loaded player health: " + currentHealth);
            }
            else
            {
                currentHealth = maxHealth; // Default to max health if no saved data
                Debug.LogWarning("No saved health found. Setting to max health.");
            }
            UpdateHealthBar();
